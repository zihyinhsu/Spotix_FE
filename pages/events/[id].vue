...
const eventData = ref<eventType>({
  ...
  imgUrl: 'https://static.tixcraft.com/images/activity/25_day6_70552ba96e820fbd83666c05d1b73da8.jpg',
  location: '台北小巨蛋',
  price: 5000,
...
const sessionsData = ref<showType[]>([
  {
    id: 1,
    name: 'DAY6 3RD WORLD TOUR ＜FOREVER YOUNG＞ in KAOHSIUNG',
...
<template>
  <div>
    <BreadCrumb :event-data="eventData" />
    ...
    <div class="container my-6 md:my-12 space-y-8">
          ...
          >
            ...
            <div class="text-right w-[200px] p-2">
              <fwb-button
                ...
                @click="() => $router.push(`/events/area/${eventData.id}?session=${session.id}`)"
              >
      ...
      >
        ...
        >
          ...
          <fwb-table-body>
            <fwb-table-row
              v-for="seession in sessionsData"
              :key="seession.id"
            >
              <fwb-table-cell
                v-for="(config, idx) in tableConfig"
              ...
              >
                <template v-if="config.value !== 'status'">
                  {{ seession[config.value] }}
                </template>
                <template v-else>
                  <fwb-button
                    primary
                    :class="seession.status ? 'bg-primary hover:bg-primary focus:ring-opacity-0 transition duration-500 ease-in-out' : 'text-primary bg-transparent p-0'"
                    :disabled="!seession.status"
                    @click="$router.push(`/events/area/${eventData.id}?session=${seession.id}`)"
                  >
                    {{ seession.status ? '立即訂購' : '已售完' }}
                  </fwb-button>
                </template>
              </fwb-table-cell>
            </fwb-table-row>
